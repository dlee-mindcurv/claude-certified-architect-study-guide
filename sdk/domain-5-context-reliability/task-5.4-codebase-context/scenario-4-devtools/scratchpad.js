...
export class Scratchpad {
  ...
  appendSection(sectionName, findings) {
    const lines = [`## ${sectionName}`, `Updated: ${new Date().toISOString()}`, ''];
  ...
  exists() { return existsSync(this.scratchpadPath); }
...
export class InvestigationPhase {
  constructor({ name, question, relevantFiles, dependsOn = [] }) {
    this.name = name;
...
export class InvestigationCoordinator {
  ...
  addPhase(phase) { this.phases.push(phase); }

  resumeFrom(savedState) {
    if (!savedState) return;
    for (const phase of this.phases) {
      if ((savedState.completedPhases || []).includes(phase.name)) {
  ...
  getNextPhase() {
    for (const phase of this.phases) {
      ...
      if (phase.dependsOn.every(dep => this.completedPhases.has(dep))) return phase;
    }
    return null;
  }
  ...
  isComplete() { return this.phases.every(p => p.status === 'completed' || p.status === 'skipped'); }
...
async function runSubagent(phase, codeContext) {
  let result = '';
